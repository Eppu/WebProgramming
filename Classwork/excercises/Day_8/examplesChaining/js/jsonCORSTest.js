...
function getJSONData(){
	//xhr.open('GET','http://lissu-api.herokuapp.com');
	//xhr.open('GET','data/testdata.json');
	xhr.open('GET','http://www.sis.uta.fi/~tohois/tamk/testdata.json');
	xhr.send();
}
...
function processJSONData(){
	...
	if (xhr.status === 200){
		...
		if(responseObject.vehicles.length !=='undefined'){
			//for(var i = 0;i < responseObject.length;i++){
			//	createDOMObject(responseObject[i]);
			//}
			display_status("number of items: " + responseObject.vehicles.length);
		} else{
